...
export async function getCurrentMonthBatch(userId: string): Promise<CurrentMonthBatch> {
  const now = new Date();
  const monthNum = now.getMonth() + 1;
  const label = `${monthNum}월DB`;
  const pattern = new RegExp(`^${now.getFullYear()}\\s*년\\s*${monthNum}\\s*월`);

  const batches = await prisma.customerBatch.findMany({ where: { userId }, select: { name: true } });
