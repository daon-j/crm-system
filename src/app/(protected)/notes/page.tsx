...
export default async function NotesPage({
  ...
  return (
    <div className="max-w-3xl">
      <h1 className="text-2xl font-bold text-ink mb-1">학습노트</h1>
      ...
      <div className="space-y-3">
          ...
          <div key={n.id} className="rounded-xl border border-border bg-surface p-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Link href={`/notes/${n.id}/edit`} className="text-xs text-ink-muted hover:text-primary">
                  수정
                </Link>
            ...
            <h3 className="text-sm font-semibold text-ink mt-2">{n.title}</h3>
            <p className="text-sm text-ink-2 mt-1 whitespace-pre-wrap">{n.content}</p>
            {n.tags && (
              <div className="flex flex-wrap gap-1 mt-2">
                {n.tags.split(",").map((t) => (
                  <span key={t} className="rounded bg-surface-muted px-1.5 py-0.5 text-[11px] text-ink-muted">
                    #{t.trim()}
                  </span>
                ))}
              </div>
            ...
            {n.attachments
              ...
              .map((a) => (
                <div key={a.id} className="mt-3 rounded-lg border border-border bg-surface-muted p-3">
                  <span className="text-xs font-medium text-ink-muted">📝 대본</span>
                  <p className="text-sm text-ink-2 mt-1 whitespace-pre-wrap">{a.content}</p>
                </div>
              ))}
        ...
        {notes.length === 0 && (
          <p className="text-sm text-ink-muted">기록된 노트가 없습니다</p>
        )}
      </div>
    </div>
  );
