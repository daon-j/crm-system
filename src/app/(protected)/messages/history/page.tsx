  ...
  return (
    <div className="max-w-3xl">
      <div className="flex items-center justify-between mb-1">
        <h1 className="text-2xl font-bold text-slate-900">발송이력 전체보기</h1>
      ...
      <form className="flex items-end gap-2 mb-6">
        <div>
          <label className="block text-xs text-slate-500 mb-1">조회 월</label>
          <select
            name="month"
            defaultValue={month ?? ""}
      ...
      <p className="text-sm text-slate-500 mb-3">{filtered.length}건</p>
      <div className="space-y-2">
        {filtered.map((m) => (
          <div key={m.id} className="rounded-lg border border-slate-100 bg-slate-50 px-3.5 py-2.5 text-sm">
            <div className="flex items-center gap-2 text-xs text-slate-400 mb-1">
              <span className="font-medium text-slate-600">{m.customer.name}</span>
              <span className="rounded bg-slate-200 px-1.5 py-0.5 text-slate-500">{m.triggerType}</span>
              <span>{formatDateTime(m.sentAt)} 발송</span>
            </div>
            <p className="text-slate-600">{m.content}</p>
          </div>
        ))}
        {filtered.length === 0 && <p className="text-sm text-slate-400">해당 기간 발송이력이 없습니다</p>}
      </div>
    </div>
  );
}
