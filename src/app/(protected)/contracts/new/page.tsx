...
export default async function NewContractPage({
  ...
  const backHref = customerId ? `/customers/${customerId}` : "/contracts";
  const backLabel = customerId ? "← 고객 상세로" : "← 계약목록";

  return (
    <div className="max-w-xl">
      <Link href={backHref} className="text-sm text-ink-muted hover:underline">
        {backLabel}
      </Link>
      <h1 className="text-2xl font-bold text-ink mb-6 mt-1">계약 등록</h1>
      <form action={createContract} className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <label className="block text-sm font-medium text-ink-2 mb-1">
              보험사 <span className="text-danger">*</span>
            </label>
            <input
              name="insurer"
              required
        ...
        <div>
          <label className="block text-sm font-medium text-ink-2 mb-1">
            상품 종류 <span className="text-danger">*</span>
          </label>
          <select
            name="category"
            required
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <label className="block text-sm font-medium text-ink-2 mb-1">
              가입일 <span className="text-danger">*</span>
            </label>
            <input
              type="date"
              name="joinDate"
          ...
          <div>
            <label className="block text-sm font-medium text-ink-2 mb-1">
              만기일 <span className="text-danger">*</span>
            </label>
            <input
              type="date"
              name="expiryDate"
