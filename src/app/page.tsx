...
export default async function DashboardPage() {
  ...
  const [
    ...
    prisma.customer.count({ where: { mobileConsent: true } }),
    prisma.customerBatch.findMany({ select: { id: true, name: true } }),
    prisma.todo.findMany(),
    prisma.customer.groupBy({ by: ["grade"], _count: true }),
    ...
    prisma.calendarEvent.findMany({
      where: { type: "VISIT", status: "SCHEDULED", startAt: { gte: todayStart, lt: todayEnd } },
      orderBy: { startAt: "asc" },
      include: {
        customer: {
          include: {
    ...
    prisma.calendarEvent.findMany({
      where: { type: "VISIT", status: "SCHEDULED", startAt: { gt: todayEnd } },
      orderBy: { startAt: "asc" },
      take: 10,
      include: {
        customer: {
    ...
    prisma.consultation.findMany({
      where: { nextContactDate: { gte: todayStart, lt: todayEnd } },
      include: { customer: true },
    ...
    prisma.contract.count({ where: { source: "AGENT_SALE", joinDate: { gte: monthStart } } }),
    getDashboardLayout(),
  ]);
  ...
  const [customersThisMonth, mobileConsentThisMonth] =
    currentMonthBatchIds.length > 0
      ? await Promise.all([
          prisma.customer.count({ where: { batchId: { in: currentMonthBatchIds } } }),
          prisma.customer.count({
            where: { batchId: { in: currentMonthBatchIds }, mobileConsent: true },
          }),
        ])
      : [0, 0];
  ...
  const miniItemsByDay = await getCalendarItems(miniStart, miniEnd);
  const weekRows = [
